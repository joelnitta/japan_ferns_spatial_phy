...
make_species_ranking_table <- function (ranking_data, motif_select, taxon_select = NULL, slice_n = 10) {
  ...
  ranking_data_sliced <- ranking_data %>%
    ...
    # a single motif)
    arrange(desc(score), desc(score_sd)) %>%
    slice(1:slice_n) %>%
    select(taxon, score) %>%
  ...
  if(!is.null(taxon_select))
    table <-
      ...
      tab_style(
        ...
        locations = cells_data(
          columns = everything(),
          rows = taxon == ranking_data_sliced$taxon[[taxon_select]])
      )
...
plot_distribution <- function (ranking_data, motif_select, taxon_select, occ_data, show_title = TRUE) {
  ...
  cells_present <-
    occ_data %>%
    filter(taxon_name == ranking_data$taxon[[taxon_select]]) %>%
  ...
  if(isTRUE(show_title))
    plot <- plot +
    labs(
      title = ranking_data$taxon[[taxon_select]]
    )
...
plot_dist_by_motif <- function(species_ranking, motif, occ_data_ferns) {
  dist_maps <- purrr::map(1:10, ~plot_distribution(species_ranking, motif, ., occ_data_ferns)) %>%
...
.pull-right-narrow[
```{r ecos-motif-1-focus-3}
make_species_ranking_table(species_ranking, 1, 3)
plot_distribution(species_ranking, 1, 3, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-1-focus-7}
make_species_ranking_table(species_ranking, 1, 7)
plot_distribution(species_ranking, 1, 7, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-2-focus-1}
make_species_ranking_table(species_ranking, 2, 1)
plot_distribution(species_ranking, 2, 1, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-2-focus-2}
make_species_ranking_table(species_ranking, 2, 2)
plot_distribution(species_ranking, 2, 2, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-3-focus-3}
make_species_ranking_table(species_ranking, 3, 3)
plot_distribution(species_ranking, 3, 3, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-3-focus-7}
make_species_ranking_table(species_ranking, 3, 7)
plot_distribution(species_ranking, 3, 7, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-4-focus-2}
make_species_ranking_table(species_ranking, 4, 2)
plot_distribution(species_ranking, 4, 2, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-4-focus-8}
make_species_ranking_table(species_ranking, 4, 8)
plot_distribution(species_ranking, 4, 8, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-5-focus-3}
make_species_ranking_table(species_ranking, 5, 3)
plot_distribution(species_ranking, 5, 3, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-5-focus-7}
make_species_ranking_table(species_ranking, 5, 7)
plot_distribution(species_ranking, 5, 7, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-6-focus-1}
make_species_ranking_table(species_ranking, 6, 1)
plot_distribution(species_ranking, 6, 1, occ_data_ferns, show_title = FALSE)
```
]
...
.pull-right-narrow[
```{r ecos-motif-6-focus-2}
make_species_ranking_table(species_ranking, 6, 2)
plot_distribution(species_ranking, 6, 2, occ_data_ferns, show_title = FALSE)
```
]

---

## Motif 2 tends to cluster, motif 6 tends to overdisperse
