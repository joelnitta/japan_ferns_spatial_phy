...
We converted any clades in the dated tree consisting of identical sequences to polytomies with zero branch length between OTUs.
...
Values of *D* range from 0 under Brownian motion to 1 under random evolution, but can exceed this range in cases of extreme clumping or overdispersion, respectively.
...
`r n_d_signif` out of `r nrow(binary_sig_results)` of qualitative traits (coded as binary) showed evidence of phylogenetic signal (*D* ≤ 0.5, significantly different from random) (`r s_table("phy-sig-binary")`).
