...
However, species richness alone cannot provide a complete picture of biodiversity.
...
One particularly valuable characteristic of this flora is the availability of data on reproductive mode (known for `r repro_count$known$total` of `r repro_count$known$total + repro_count$unknown$total` native pteridophyte taxa excluding hybrids [`r repro_count$known$percent`]) [@Ebihara2019b].
Such densley sampled reproductive data for a large clade on the regional scale is rare, and we are unaware of any other comparable dataset in ferns.
...
In ferns, clonally reproducing apomictic species are often polyploid hybrids that share identical plastid genotypes among taxa and within populations.
...
Besides habitat loss due to human activity, one major threat to ferns in Japan is herbivory by native Japanese deer (*Cervus nippon*).
Japanee deer have caused extensive damage to native plant communities due to rapid population growth since the 1970s [@Takatsuki2009].
Extirpation of fern populations due to expansion of deer range has been observed in multiple sites across the country.
Furthermore, 33 of 212 (15.5%) fern taxa listed as endangered (Red List status CR, EN or VU) in Japan include herbivory by deer as one of the reasons for their endangered status, and one taxon (**Arachniodes yasu-inouei** var. **angustipinnula**) is thought to have gone extinct due to deer herbivory, among other causes [@MOJ2015; @kankyosho2022].
...
The vast majority of grid-cells fell to a subset of these bioregions: using taxonomic distances, they include bioregion 1 (Hokkaido and northern Honshu), bioregion 2 (southern Honshu, Kyushu, and Shikoku), bioregion 3 (Ryukyu Islands), and bioregion 4 (Ogasawara Islands) (`r figure("bioregions")`A; `r s_figure("dendrogram")`A).
Phylogenetic distances produced similar results, but the Ryukyu and Ogasawara Islands merged into a single bioregion (3), and bioregion 1 extended slightly further south (`r figure("bioregions")`B; `r s_figure("dendrogram")`B).
...
The latitudinal hump-shaped pattern of species richness observed in ferns here (Figs. `r figure_num("raw-diversity")`a, `r s_figure_num("raw-lat")`a) is well-documented [@Kusumoto2017; @Ebihara2019b], and similar patterns have been observed in deciduous broadleaved trees and conifers [@Shiono2015], and niche-modeled distributions of woody plants in Japan [@Fukaya2020].
...
The similarity in patterns of PD and FD is likely due to the moderate degree of phylogenetic signal present in the traits used in our analysis (Tables `r s_table_num("phy-sig")`, `r s_table_num("phy-sig-binary")`), and suggests that in ferns, phylogeny can be used as a reasonable stand-in for functional diversity.
Given their high phylogenetic diversity, it is perhaps not surprising that southern islands also host high amounts of phylogenetic endemism; indeed, the vast majority of cells with significant levels of PE, as well as paleoendemic cells, are located in the southern islands (Figs. `r figure_num("endemism")`, `r s_figure_num("endemism-restricted")`).
