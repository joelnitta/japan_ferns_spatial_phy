...
Conservation status (% protected area) was generally high for grid-cells with significantly high biodiversity across all biodiversity metrics, but the threat due to herbivory by native Japanese deer varied by biodiversity metric.
...
Although peak areas of different diversity types are largely non-overlapping, each one has relatively high protection coverage at medium protection level. 
Despite this, future conservation efforts must recognize that threats can vary by biodiversity metric, and consider threats not directly analyzed here such as climate change.

<!-- may include title words -->
**Key words:** apomixis, biodiversity, biogeography, conservation, CANAPE, ferns, Japan, phylogenetic diversity. 
...
We conducted phylogenetic analysis with `r software("IQ-TREE")` v1.6.12 [@Nguyen2015] and molecular dating with `r software("treePL")` v1.0 [@Smith2012] using `r n_fossil_points` fossil calibration points after @Testo2016a, with the exception of treating fossil *Kuylisporites* as belonging to crown *Cyathea*, not *Cyathea* + *Alsophila* [@Loiseau2020]. 
...
We downloaded SHP files of protected areas in Japan from the Japan Ministry of the Environment (https://www.biodic.go.jp/biodiversity/activity/policy/map/map17/index.html) and Ministry of Land, Infrastructure, Transport and Tourism (https://nlftp.mlit.go.jp/ksj/gml/datalist/KsjTmplt-A45.html). 
We only considered areas that include protection of plants, and categorized protected areas as either "high" (no human activities allowed at all) or "medium" status (some economic activities allowed by permit) [@Kusumoto2017]. 
Any areas that overlapped within a protection status were combined, and areas that overlapped between medium and high status were considered only high status.
...
Cells with significantly high biodiversity had a greater percentage of area protected than the overall rate for Japan at the medium protection status (`r scales::percent(total_percent_protected_medium, accuracy = 0.1)`) across all biodiversity metrics.
In contrast, cells with significantly high biodiversity had a lower percentage of area protected than the overall rate for Japan at the high protection status (`r scales::percent(total_percent_protected_medium, accuracy = 0.1)`) for most biodiversity metrics.
Only cells with significantly high PE had a higher percentage area protected (`r best_protected_high` high, `r best_protected_med` medium, `r best_protected_total` total) than the overall rate at both levels of protection.
...
Cells with high FD, PD, or PE tended to have less deer occupation compared to the overall rate (`r figure("conserv-status")`b).
However, cells with high species richness consistently exceeded the overall rate of deer presence, with up to `r deer_highest_danger$estimated$percent` occupied by deer in the estimated range dataset.
...
The Tsugaru Strait had no relation to any of the bioregions, and although the Tokara Strait somewhat splits bioregions 2 and 3, there is some overlap between them on the Amami-Oshima Isl. (`r figure("bioregions")`). 
...
One possible reason for this generally high level of protection is that in the past, some ferns have been included in lists of rare and endemic plants for consideration in designation of conservation areas (A. Ebihara, pers. comm.). 
In particular, there has been an emphasis on conserving rare and endemic species in Japanese southern islands such as Ogasawara and Amami (FIXME: add ref), which include many grid-cells with high PE.
Another reason (as suggested previously) is that ferns may serve as bioindicators reflecting overall levels of biodiversity, particularly for other plant groups. 
...
This pattern is due to the fact that Japanese deer do not occur in the southern islands, which house high amounts of PD and PE; rather, areas with high species richness are primarily located in southern Honshu, where deer expansion has been intense (FIXME: add SI fig).
...
b <- signif_cells_deer_danger %>%
  ...
  geom_col(position = position_dodge()) +
  # line for 1978 overall percent
  geom_vline(
    xintercept = total_percent_deer_danger %>% filter(range == "1978") %>% pull(percent_danger), 
    color = "grey40", linetype = 3) +
  # line for 2003 overall percent
  geom_vline(
    xintercept = total_percent_deer_danger %>% filter(range == "2003") %>% pull(percent_danger), 
    color = "grey40", linetype = 2) +
  # line for estimated overall percent
  geom_vline(
    xintercept = total_percent_deer_danger %>% filter(range == "estimated") %>% pull(percent_danger), 
    color = "grey40", linetype = 1) +
  scale_fill_manual(
    values = deer_colors,
    guide = guide_legend(
      direction = "horizontal",
...
**`r figure("conserv-status")`**. Conservation status and threat due to herbivory by deer for ferns of Japan by biodiversity metric. (a) Percent of land area with protected status for grid cells with significantly high biodiversity. (b) Percent of land area occupied by deer for grid cells with significantly high biodiversity. Biodiversity metrics include phylogenetic diversity (PD), functional diversity (FD), and phylogenetic endemism (PE). Significance of biodiversity metrics assessed by a one-tailed test comparing observed values to a null distribution of 999 random values (see Materials and Methods). For (a), vertical lines indicate total percent area protected across Japan (baseline protection rate); dashed, high protection level (`r scales::percent(total_percent_protected_high, accuracy = 0.1)`); dotted, medium protection level (`r scales::percent(total_percent_protected_medium, accuracy = 0.1)`). For (b), vertical lines indicate total percentage of land area occupied by deer across Japan (baseline threat rate); solid, percentage estimated with a model based on 2003 survey data (`r deer_overall$estimated$percent`); dashed, percentage based on 2003 survey data (`r deer_overall[["2003"]]$percent`); dotted, percentage based on 1978 survey data (`r deer_overall[["1978"]]$percent`).
