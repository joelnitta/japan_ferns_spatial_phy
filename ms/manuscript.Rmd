...
treePL requires branch lengths to be > 0, and it resets extremely small branches an arbitrary minimum positive length.
...
We converted any clades in the original dated tree consisting of identical sequences to polytomies with zero branch length between OTUs.
...
Previous vegetation zone schemes in Japan have typically included 5--6 zones [@Shimizu2014]; to enable approximate comparison with these, we tested values of *k* from 1 to 10, and selected the optimal value using the `optimal_phyloregion` function in the R package `r pack("phyloregion")`.
...
Phylogenetic distances produced similar results, but the Ryukyu and Ogasawara Islands merged into a single bioregion, and bioregion 1 extended slightly further south (`r figure("bioregions")`b; `r s_figure("dendrogram")`b). 
