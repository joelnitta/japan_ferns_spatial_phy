...
All spatial models were significant relative to null models without any fixed effects. The effect of temperature dominated in most models, with only SES of PD showing a stronger effect of precipitation seasonality (`r figure("coeff")`A--E). Biodiversity metrics with a genetic component (SES of PD and SES of RPD) each had a large, negative relationship with % apomictic taxa (`r figure("coeff")`F--G). Conditional AIC was lower (indicating better model fit) in the reproductive model compared to the environmental model for SES of RPD, but not SES of PD.
...
model_params_for_plot <-
  ...
  # Make coefficient with greatest abs value bold
  group_by(resp_var, model_type) %>%
  mutate(
    term_print = case_when(
      abs(estimate) == max(abs(estimate)) ~ glue::glue("**{term_print}**"),
      TRUE ~ term_print
    ),
...
plot_coeff <- function (df) {  
  ...
  ggplot(plot_data, aes(x = estimate, y = term_print, color = signif)) +
    geom_vline(xintercept = 0, linetype = "dotted") +
    geom_pointrange(aes(xmin = estimate - cond_se, xmax = estimate + cond_se), size = 0.25) +
...
**`r figure("coeff")`**. Regression coefficients of general mixed models. A--E, environmental models (models including temperature); F and G, reproductive models (models including % apomictic taxa). Error bars show 95% confidence interval. Statistical significance assessed with a likelihood ratio test (LRT) between the full model and a model with the focal independent variable removed (null model); *p*-value indicates probability of no difference between the full and null model. Name of response variable and model type indicated above each subplot. Independent variable with greatest absolute coefficient value in bold for each model.
