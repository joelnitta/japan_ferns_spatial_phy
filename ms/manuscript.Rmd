...
FIXME: add acknowledgements (not including reviewers)
...
**Key words:** apomixis, biodiversity, biogeography, ferns, Japan, phylogenetic diversity. 
...
Our data include all ferns known to occur within the boundaries of Japan (20°N to 45°N, 122°E to 153°E). 
...
We conducted phylogenetic analysis with `r software("IQTREE")` [@Nguyen2015] and molecular dating with `r software("treePL")` [@Smith2012] using `r nrow(plastome_calibration_dates)` fossil calibration points after @Testo2016a. 
We then trimmed the tree to Japanese taxa only and used this for all further analyses.
Tree visualization was done with the `r pack("ggtree")` R package [@Yu2017].
...
**`r figure("bioregions")`**. Bioregions of the ferns of Japan. (a), taxonomic bioregions. (b), phylogenetic bioregions. Bioregions determined by clustering taxonomic (Sørensen) or phylogenetic (PhyloSor) distances into *k* = 8 or 7 groups, respectively.
...
Data files, including the DNA alignment, phylogenetic tree, and community data matrix, are openly available from Dryad at http://doi.org/ADD_DOI_HERE_WHEN_ASSIGNED.
...
Author contributions: J.H.N., B.M., and A.E. conceived the ideas; A.E. provided the data; J.H.N. analyzed the data; I.W. provided computational resources; J.H.N. wrote the manuscript with input from all co-authors.
