...
All biodiversity metrics except for SES of RFD showed a significant relationship with temperature (`r table("env-model-params")`). 
...
```{r env-model-params-prep}
# Format model parameter summary for nice printing as a table
...
env_model_params %>%
  ...
  select(resp_var, term, estimate, cond_se, t_value, p_value) %>%
  ...
  arrange(resp_var, term) %>%
  select(resp_var, term, estimate, cond_se, t_value, p_value, signif)
```
...
```{r endemism-save, fig.width = 5, fig.height = 5}
...
```{r conserv-status-save, fig.width = 4, fig.height = 4}
