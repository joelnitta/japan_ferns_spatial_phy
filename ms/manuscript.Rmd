...
**Methods** We used data from >`r nrow(occ_point_data_ferns) %>% plyr::round_any(1000, f = floor) %>% number(big.mark = ",")` fern specimens in Japan to compile a community dataset of `r nrow(comm_ferns) %>% scales::number(big.mark = ",")` 0.2° grid-cells including `r ncol(comm_ferns)` native, non-hybrid taxa. 
...
To obtain a maximally sampled, robust phylogenetic tree, we combined the *rbcL* data of @Ebihara2019b with a globally sampled plastid dataset including all fern sequences on GenBank for four plastid genes (*rbcL*, *atpB*, *aptA*, and *rps4*) and 66 other coding, single-copy plastid genes extracted from 100 complete fern plastomes (Nitta et al., in prep) (`r ape::Ntip(plastome_tree) %>% scales::number(big.mark = ",")` OTUs total). 
...
assert_that(phy_sig_results %>% pull(kval) %>% max < 0.17)
...
All continuous traits showed significant phylogenetic signal, but estimates of signal strength differed between *K* and lambda: all lambda values were > 0.95, indicating evolution by BM; however, values of *K* were < 0.17, indicating less phylogenetic signal than expected under BM (`r s_table("phy-sig")`). 
...
A smaller number of cells had significantly high PD, and these were located mostly on smaller southern (Ryukyu, Izu, Ogasawara) islands or coastal areas (Izu Peninsula, Kii Peninsula). 
...
Small clusters of significantly endemic cells were also observed northwest of Tokyo (Nagano prefecture), and in Hokkaido (`r figure("endemism")`). 
When we ran CANAPE with a reduced dataset including only taxa absolutely endemic to Japan to account for the border effect, there were fewer significant cells, but a similar pattern of significantly endemic cells mostly occurring in the southern islands was observed (`r s_figure("endemism-restricted")`).
...
The vast majority of grid-cells fell to a subset of these clusters: using taxonomic distances, they include Cluster 1 (Hokkaido and northern Honshu), Cluster 2 (southern Honshu), Cluster 3 (Ryukyu Islands, excluding Yakushima), and Cluster 4 (Ogasawara Islands) (`r figure("bioregions")`A; `r s_figure("dendrogram")`A). 
Phylogenetic distances produced similar results, but the Ryukyu and Ogasawara Islands merged into a single cluster (`r figure("bioregions")`B; `r s_figure("dendrogram")`B). 
...
Furthermore, we identified % apomictic taxa as a strong driver of SES of RPD (`r figure("modelfit")`G).
...
We verified that this relationship is also observed when analyzed with a phylogeny with branch lengths in units of genetic change (`r ie`, the output of the maximum likelihood phylogenetic analysis, before dating), consistent with an effect on genetic diversity (`r s_figure("modelfit-not-ult")`B).
...
ggsave("plot.pdf", japan_map_plot, width = 5, height = 5, units = "in")
```
...
**`r figure("endemism")`**. Phylogenetic endemism of the ferns of Japan measured using CANAPE (categorical analysis of neo- and paleo-endemism). 'paleo' indicates paleoendemic areas (those with an overabundance of long branches); 'neo' indicates neodendemic areas (those with an overabundance of short branches); 'mixed' indicates significantly endemic sites that have neither an overabundance of short nor long branches; 'super' indicates highly significantly endemic sites that have neither an overabundance of short nor long branches. For details, see Materials and Methods.
