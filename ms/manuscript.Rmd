...
**Premise:** Biodiversity is often measured with a single metric, species richness. However, this ignores evolutionary history and is not sufficient for making conservation decisions. Here, we use a densely sampled dataset to characterize multiple facets and drivers of biodiversity and understand how these relate to bioregions and conservation status.

**Methods:** We used data from >`r occ_sum$filtered$n_specimens$value %>% plyr::round_any(1000, f = floor) %>% number(big.mark = ",")` fern specimens in Japan to compile a community dataset of `r nrow(comm_ferns) %>% scales::number(big.mark = ",")` 0.2° grid-cells including `r ncol(comm_ferns)` taxa. 
