...
```{r macros}
# Define formattting for some common custom words that may vary in style between journals
ie <- "*ie*"
eg <- "*eg*"
# R packages
...
pagebreak_doc <- function() {ifelse(params$doc_type == "doc", run_pagebreak(), return(""))}
# Pagebreak for pdf only
pagebreak_pdf <- function() {ifelse(params$doc_type == "pdf", return("\\clearpage"), return(""))}
# Pagebreak for both
pagebreak <- function() {
  if (params$doc_type == "doc") {
    run_pagebreak()
  } else if (params$doc_type == "pdf") {
    return("\\clearpage")
  } else {
...
We constructed a linear model for each response variable (richness, SES of PD, SES of FD, SES of RPD, SES of RFD, PE p-score) dependent on the environmental variables. We did not include any interaction terms as we had no a-priori reason to do so. Initial inspection of the relationship between richness and temperature showed a hump-shaped pattern, so we also included a quadratic term for temperature for the richness model only. We fit models including a Matérn correlation matrix based on the geographic centroids of each grid cell as a random effect with the R package spaMM [@Rousset2014]. We verified the significance of each term of the model by conducting likelihood ratio tests (LRTs) between the full model and models each with a single term of interest removed, in sequence. We also verified the significance of the full model relative to the null model (only including the Matérn correlation matrix) using LRTs.

We postulated that % apomictic taxa might explain the distribution of genetic diversity better than temperature, so we also constructed a limited set of models to test this hypothesis. These models included response variables with a genetic component (SES of PD, SES of RPD, PE p-score) as dependent on either % apomictic taxa or temperature, precipitation, and precipitation seasonality, with random spatial effects as described above. We compared the fit between models including % apomictic taxa or temperature using conditional Akaike Information Criterion (cAIC).
