...
**Methods** We used a list of >300,000 fern specimens in Japan to compile a community dataset of `r nrow(comm_ferns) %>% scales::number(big.mark = ",")` 0.2° grid-cells including `r ncol(comm_ferns)` native, non-hybrid taxa. We combined this with a phylogenetic tree and functional trait data to analyze taxonomic, phylogenetic, and functional diversity, and assessed significance using a null distribution of 999 random communities. We conducted categorical analysis of neo- and paleo-endemism to identify areas of significant phylogenetic endemism. We used hierarchical clustering of taxonomic and phylogenetic distances to analyze bioregions. We compared the overlap of significantly diverse grid-cells with existing conservation zones to assess conservation status.
...
We downloaded climate data at the 2.5 minute scale from the WorldClim database using the `getData` function in the R package `raster`, and extracted four variables related to our research questions as follows: mean annual temperature (bio1), temperature seasonality (bio4), annual precipitation (bio12), and precipitation seasonality (bio15). We checked for correlation between climatic variables while taking into account spatial autocorrelation using a modified *t*-test with the `modified.ttest` in the `SpatialPack` R package; as a result, we excluded temperature seasonality, which was correlated with mean annual temperature. We identified the intersection of the climate data at the 2.5 minute scale with the 0.2° grid-cells, then calculated the mean of each climatic variable for each 0.2° grid-cell. The R package 'sf' was used for GIS analyses.
...
We tested for a relationship between SES of each biodiversity metric and reproductive mode (% apomictic taxa) initially with linear models constructed using the R function 'lm'. Next, we checked for evidence of spatial autocorrelation in the linear model residuals with Moran's I using 'moran.mc' function in the spdep package. Spatial autocorrelatoin was frequently detected, so to acccount for this we constructed spatial linear mixed models including a Matérn correlation matrix based on the geographic centroids of each grid cell with the R package spaMM [@Rousset2014]. We then again checked for any remaining spatial autocorrelation in the mixed models with Moran's I.
