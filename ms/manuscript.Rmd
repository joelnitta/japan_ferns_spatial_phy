...
### Overlap with protected zones

We downloaded SHP files of protected areas in Japan from the Japan Ministry of the Environment (https://www.biodic.go.jp/biodiversity/activity/policy/map/map17/index.html). We categorized protected areas as either "high" (plants protected; no human activities allowed at all) or "normal" (plants protected; some human activities allowed by permit). We cropped the grid cells to only include land area, then calculated the amount of protected area of each status type within grid cells with significantly high PD, FD, or PE (one-tailed test).
...
### Number of specimens and redundancy

The distribution of specimen collection locations is highly skewed, with >3,000 specimens per grid cell northwest of Tokyo in Saitama prefecture, and many fewer elsewhere (`r figure("abundance")`A). Despite this, sampling redundancy is generally high throughout the country, with areas of low redundancy in Hokkaido and some coastal areas and small islands (`r figure("abundance")`B). The grid cells that had to be excluded from the analysis due to low redundancy were mostly from Hokkaido.
...
Taxonomic clusters 3 and 4 have higher SES values of PD, RPD, FD, and RFD than clusters 1 and 2. Clusters 3 and 4 also have consistently high PE *p*-scores, whereas clusters 1 and 2 have a much wider variation. Cluster 2 has lower RPD and RFD than cluster 1. Comparison of biodiversity metrics across phylogenetic clusters was similar, except that phylogenetic cluster 3 mostly corresponds to taxonomic clusters 3 and 4 combined.

### Degree of protection

## Discussion

Here, we provide the first insights into the combined taxonomic, phylogenetic, and functional diversity of the ferns of Japan. Our integrative approach reveals key differences in patterns of biodiversity in different parts of the archipelago. 

### Distribution of biodiversity

The latitudinal hump-shaped pattern of species richness observed in ferns here has been well-documented.

Despite the peak of richness in southern Kyushu, the overwhealming pattern driving phylogenetic and functional diversity is the high diversity of southern islands (Ryukyu and Bonin Isl.).

Another major pattern revealed here is the predominance of long branches in the northern half of Honshu, and the predominance of short branches in the southern half, particularly Kyushu.
...
japan <- rnaturalearth::ne_countries(country = "japan", scale = "large", returnclass = "sf") %>%
  sf::st_crop(sf::st_bbox(biodiv_ferns_spatial))
...
**`r figure("abundance")`**. `r figure_cap("abundance")`
