...
tibble(taxon = plastome_tree$tip.label) %>%
  anti_join(select(plastome_sampling, taxon) %>% unique, by = "taxon") %>%
...
# Save plot if rendering to doc
ggsave(result_file(figure("raw-diversity"), "pdf"), width = 7.25, height = 4, units = "in", raw_diversity_plot)
```
...
# Save plot if rendering to doc
ggsave(result_file(figure("rand-diversity"), "pdf"), rand_diversity_plot, width = 7.25, height = 8, units = "in")
```
...
indep_var_lookup <-
  tibble(
    term = c("lat_area", "I(temp^2)", "null_model", "percent_apo", "precip", "precip_season", "temp"),
...
# Save plot if rendering to doc
ggsave(result_file(figure("coeff"), "pdf"), coeff_plot, width = 7.25, height = 5, units = "in")
```
...
# Save plot if rendering to doc
ggsave(result_file(figure("modelfit"), "pdf"), modelfit_plot, width = 7.25, height = 5, units = "in")
```
...
# Save plot if rendering to doc
ggsave(result_file(figure("bioregions"), "pdf"), bioregions_plot, width = 7.25, height = 5, units = "in")
```
...
# Save plot if rendering to doc
ggsave(result_file(figure("div-by-tax-region"), "pdf"), div_by_tax_region_plot, width = 7.25, height = 7, units = "in")
```
...
# Save plot if rendering to doc
ggsave(result_file(figure("conserv-status"), "pdf"), conserv_status_plot, width = 7.25, height = 4, units = "in")
```
