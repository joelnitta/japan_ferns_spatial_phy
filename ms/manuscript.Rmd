...
**Methods:** We compiled a community dataset of `r nrow(comm_ferns) %>% scales::number(big.mark = ",")` 0.2° grid-cells including `r ncol(comm_ferns)` taxa based on > `r occ_sum$filtered$n_specimens$value %>% plyr::round_any(1000, f = floor) %>% number(big.mark = ",")` specimen records.
...
Furthermore, the overall sampling completeness of this dataset is extremely high (`r s_figure("sampling-curve")` in Appendix S1; see Supplemental Data with this article). 
...
Latitude and longitude of each occurrence record were projected into the Japanese Geodetic Datum 2000 (JGD 2000), which was also used for all further geographic analyses.
...
We calculated area as rolling mean area (km^2^) across 1° latitudinal bands.
...
Initial inspection of the biodiversity and environmental data showed a single outlier in % apomictic taxa due to extremely low species richness (Appendix S2; see Supplemental Data with this article), so that grid-cell and one grid-cell lacking environmental data were excluded from modeling analyses.
...
The other much smaller bioregions ("singletons" including only a single grid-cell each) are likely artifacts of insufficiently low taxon richness for robust clustering, and are not discussed further.
...
taxon_with_widest_lat <- lat_span_summary %>%
  ...
  left_join(
    tibble(
      ...
      text = "*Pteridium aquilinum* (L.) Kuhn subsp. *japonicum* (Nakai) A. et S. Löve"
    ),
    by = "taxon"
  ) %>%
  # Round off latitude
...
**`r s_figure("envir")`**. Reproductive mode and environmental data in 0.2° grid cells across Japan.  
**`r s_figure("tree")`**. Time tree of native Japanese ferns and lycophytes, excluding nothotaxa.  
**`r s_figure("abundance")`**. Observed number of specimens and sampling redundancy per 0.2° grid cell in the ferns of Japan.  
...
**`r figure("bioregions")`**. Bioregions of the ferns of Japan. (A) Taxonomic bioregions. (B) Phylogenetic bioregions. Bioregions determined by clustering taxonomic (Sørensen) or phylogenetic (PhyloSor) distances between grid-cells. Bioregions not consisting of > 1 grid-cell each are lumped into the "Other" category.
