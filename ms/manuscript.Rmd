...
FIXME: add acknowledgements (not including reviewers) JSPS KAKENHI Grant Number 16H06279 
...
All computational analyses were carried out in R v`r getRversion()` [@RCoreTeam2020] unless otherwise stated. The `r pack("targets")` R package was used to maintain analysis workflow [@Landau2021].
...
We conducted phylogenetic analysis with `r software("IQTREE")` v1.6.12 [@Nguyen2015] and molecular dating with `r software("treePL")` v1.0 [@Smith2012] using `r nrow(plastome_calibration_dates)` fossil calibration points after @Testo2016a, with the exception of treating fossil *Kuylisporites* as belonging to crown *Cyathea*, not *Cyathea* + *Alsophila* [@Loiseau2020]. 
...
a <- ggplot() +
  geom_sf(data = japan, fill = "white", color = "transparent", lwd = coast_lwd) +
  geom_sf(data = biodiv_ferns_spatial, aes(fill = taxonomic_cluster), color = "transparent") +
  geom_sf(data = japan, fill = "transparent", color = coast_col, lwd = coast_lwd) +
...
b <- ggplot() +
  geom_sf(data = japan, fill = "white", color = "transparent", lwd = coast_lwd) +
  geom_sf(data = biodiv_ferns_spatial, aes(fill = phylo_cluster), color = "transparent") +
  geom_sf(data = japan, fill = "transparent", color = coast_col, lwd = coast_lwd) +
