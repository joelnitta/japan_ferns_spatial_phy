...
Conservation status (% protected area) was generally high for grid-cells with significantly high biodiversity across all biodiversity metrics, but the threat due to herbivory by native Japanese deer varied by biodiversity metric, and was particularly severe for taxonomic richness.
...
We conducted maximum-likelihood phylogenetic analysis with `r software("IQ-TREE")` v1.6.12 [@Nguyen2015] under the GTR+I+G model (all sites concatenated into a single data matrix), and assessed node support with 1000 ultrafast bootstrap replicates [@Minh2013].
