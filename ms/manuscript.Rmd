...
# PD
g <- ggplot(ses_div_ferns_spatial, aes(fill = pd_obs)) +
  geom_sf(color = "transparent") +
  labs(fill = "MMD") +
  scale_y_continuous(labels = scales::label_number(suffix = "\u00b0N", accuracy = 1)) +
  scale_x_continuous(labels = scales::label_number(suffix = "\u00b0E", accuracy = 1)) +
...
# SES of PD
h <- ggplot(ses_div_ferns_spatial, aes(fill = pd_obs_z)) +
  geom_sf(color = "transparent") +
  labs(fill = "SES PD") +
  scale_fill_scico(
    palette = "romaO",
...
# To add: RPD
# ggplot(ses_div_ferns_spatial, aes(fill = rpd_obs_z)) +
#   geom_sf(color = "transparent") +
#   labs(fill = "SES RPD") +
#   scale_fill_scico(
#     palette = "romaO",
...
#   scale_x_continuous(labels = scales::label_number(suffix = "\u00b0E", accuracy = 1)) +
#   # highlight only significant cells
#   # gghighlight(rpd_obs_p > 0.95 | rpd_obs_p < 0.05) +
#   map_theme +
#   theme(
#     axis.text.y = element_blank()
#   )
...
**`r figure("diversity")`**. `r figure_cap("diversity")` **A**, richness. **B**, sampling redundancy. **C**, mean phylogenetic distance (MPD). **D**, Standard effect size (SES) of MPD. **E**, mean morphological distance (MMD) **F**, SES of MMD. **G**, phylogenetic diversity (PD) **H**, SES of PD.
...
# SES PE
b <- ggplot(ses_div_ferns_spatial, aes(fill = pe_obs_p)) +
  geom_sf(color = "transparent") +
  labs(fill = "SES PE") +
  scale_fill_viridis_c() +
  scale_y_continuous(labels = scales::label_number(suffix = "\u00b0N", accuracy = 1)) +
