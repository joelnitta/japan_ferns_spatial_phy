...
All measures of biodiversity and CANAPE were carried out using the `r pack("canaper")` package [@Nitta2021], which calculates raw biodiversity metrics using functions in the `r pack("phyloregion")` package [@Daru2020a] and randomizations using the `r pack("picante")` package [@Kembel2010].
...
Data are openly available from Figshare at http://doi.org/ADD_DOI_HERE_WHEN_ASSIGNED.
...
Rather, fern bioregions in Japan broadly separate along gradients of temperature and precipitation; in particular, precipitation splits taxonomic Cluster 3 (mostly the Ryukyu islands, wetter) from Cluster 4 (mostly the Ogasawara islands, drier) (`r s_figure("bioregions-env")`).
