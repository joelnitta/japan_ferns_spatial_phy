...
phy_sig_results %>%
  ...
  mutate(lambda_pval = format(lambda_pval, digits = 3)) %>%
  write_csv(path = result_file(s_table("phy-sig"), "csv"))
```

**`r s_table("phy-sig")`**. `r s_table_cap("phy-sig")` K, Blomberg's *K*; lambda, Pagel's lambda.

```{r phy-sig-binary}
binary_sig_results %>%
  arrange(trait) %>%
  write_csv(path = result_file(s_table("phy-sig-binary"), "csv"))
```

**`r s_table("phy-sig-binary")`**. `r s_table_cap("phy-sig-binary")` num_present, number of times a trait was observed present; num_absent, number of times a trait was observed absent; D, Fritz and Purvis' *D*; prob_random, probability of obtaining observed trait distribution given random distribution of traits; prob_brownian, probability of obtaining observed trait distribution given random evolution by Brownian Motion. Presence or absence of indusium was originally a binary trait; other traits (margin, shape, texture) are qualitative traits scored in binary format.
...
**`r s_figure("abundance")`**. `r s_figure_cap("abundance")`
