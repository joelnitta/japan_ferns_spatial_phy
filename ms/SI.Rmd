...
loadd(ses_phy_ferns, cache = ja_fern_cache)
loadd(ses_traits_ferns, cache = ja_fern_cache)
loadd(shape_ferns, cache = ja_fern_cache)

# Helper function for making a scatterplot
scatterplot <- function(data, x_var, y_var, x_lab, y_lab) {
...
ses_comp <-
ses_phy_ferns %>%
  select(site, mpd_obs, pe_obs, mpd_obs_z, pe_obs_z) %>%
  left_join(
    select(ses_traits_ferns, site, mpd_morph_obs, mpd_morph_obs_z),
    by = "site"
  ) %>%
  left_join(
    as_tibble(shape_ferns) %>% select(site = grids, richness),
    by = "site"
  )
...
# Make plot
pmap(raw_comparisons, scatterplot, data = ses_comp) %>%
wrap_plots() +
  plot_annotation(tag_levels = 'A')
...
# Make plot
pmap(ses_comparisons, scatterplot, data = ses_comp) %>%
  wrap_plots() +
  plot_annotation(tag_levels = 'A')
```
