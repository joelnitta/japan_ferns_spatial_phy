...
bibliography: 
  - /home/rstudio/japan_ferns_biogeo/repos/japan_ferns_biogeo/ms/references.yaml
  - /home/rstudio/japan_ferns_biogeo/repos/japan_ferns_biogeo/ms/references_other.yaml
...
binary_sig_results %>%
  ...
  rename(
    ` ` = trait,
    `*n* (present)` = num_present,
    `*n* (absent)` = num_absent,
    `*p* (random)` = prob_random,
    `*p* (Brownian)` = prob_brownian
    ) %>%
  as_flextable() %>%
  colformat_md(part = "all")
```
...
lrt_results %>%
  ...
  # colformat_md() doesn't work on grouped tables
  # group_by(model_type) %>%
  ...
  rename(
    ...
    `χ^2^` = chi2_LR,
    `logL (full model)` = loglik_full,
    `logL (null model)` = loglik_null,
    `*p*` = p_value
  ) %>%
  as_flextable %>%
  merge_v(j = "Model type") %>% 
  valign(valign = "top") %>% 
  colformat_md(part = "all")
```
