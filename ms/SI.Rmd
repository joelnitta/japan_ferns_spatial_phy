...
biodiv_ferns_cent_raw <- spatial_to_cent(
  biodiv_ferns_spatial,
...
clusters_tax <-
  ...
  mutate(cluster = levels(cluster)[cluster])
...
cluster_nodes_tax_tbl <- map_df(unique(clusters_tax$cluster), ~map_cluster_to_nodes(dendro_tax, clusters_tax, .)) %>%
  ...
  mutate(cluster = as.factor(cluster) %>% fct_lump_min(4))
...
**`r s_figure("dendrogram")`**. Bioregion dendrograms. Tips are sites (names not shown). Dendrograms generated by clustering taxonomic (Sørensen) or phylogenetic (PhyloSor) distances between sites (see Materials and Methods). Color shows bioregion; bioregions with three or fewer grid cells are lumped into the "Other" category. (a), taxonomic bioregions. (b), phylogenetic bioregions. Dendrograms visualized with the `r pack("ggtree")` R package [@Yu2017].
\clearpage
...
make_protected_plot <- function(part, variable, ...) {
  ggplot() +
    geom_sf(data = japan, lwd = 0.10, fill = "white") +
    ...
    geom_sf(
      ...
      fill = "transparent", color = "black", lwd = 0.20) +
    annotate(
      geom="richtext", y = Inf, x = -Inf, 
