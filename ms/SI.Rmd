...
header-includes: 
  ...
  - \fancyhead[RE,RO]{Nitta et al.---\textit{American Journal of Botany} 2021 -- Appendix S1} # add header on right side
...
**`r s_figure("tree")`**. Time tree of native Japanese ferns and lycophytes, excluding nothotaxa. Points at nodes indicate bootstrap support. Scalebar 100 my. Tree visualized with the `r pack("ggtree")` R package [@Yu2017].
...
# Convert spatial data frame to centroids with raw metrics, drop single high apomixis outlier
biodiv_ferns_cent_raw <- spatial_to_cent_for_model(
  biodiv_ferns_spatial,
  c("grids", "lat", "long", "richness", "pd_obs", "fd_obs"))
...
**`r s_figure("dendrogram")`**. Bioregion dendrograms. Tips are sites (names not shown). Dendrograms generated by clustering taxonomic (Sørensen) or phylogenetic (PhyloSor) distances between sites (see Materials and Methods). Color shows bioregion; bioregions not consisting of > 1 grid-cell each are lumped into the "Other" category. (A), taxonomic bioregions. (B), phylogenetic bioregions. Dendrograms visualized with the `r pack("ggtree")` R package [@Yu2017].
\clearpage
...
**`r s_figure("apo-genus")`**. Rate of apomixis in genera Japanese ferns with > 1 apomictic taxon. Numbers next to each bar show number of apomictic taxa out of total richness per genus. Excludes non-native taxa and nothotaxa.
