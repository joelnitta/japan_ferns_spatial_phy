---
title: "Appendix S2: Exploratory Data Analysis"
...
header-includes: 
  ...
  - \fancyhead[RE,RO]{Nitta et al.---\textit{American Journal of Botany} 2021 -- Appendix S2} # add header on right side
...
Here we perform exploratory data analysis to determine the appropriate models to fit to the data. 
...
biodiv_ferns_cent_raw %>% 
  slice(error_rows) %>% 
  select(grids, where(~any(is.na(.x))))
```

The missing data was due to one grid-cell that lacks environmental data. This must be missing from the WorldClim dataset. Let's remove it, then continue.
